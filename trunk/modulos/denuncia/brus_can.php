...
if ($_POST['guardar']=='Guardar'){
	...
	$usuario=$_ses_user['name'];
			...
			$query="insert into epi.brucel_can
			   	(id_bru, id_denuncia, ape_pac, nom_pac, f_nacimiento,sexo,domicilio,localidad,departamento,trurales,e_frogorifico,
				...
				bili_direc,tgp,cpk,a_domestico,roedores,rio_arroyo,laguna,alcantarilla,inundacion,semana_epi,serologia,positividad,titulo,aislamiento,obs,desempleado)
			   	values
			    (nextval('epi.brucel_can_id_bru_seq'), '$id_denuncia','$ape_pac', '$nom_pac', '$f_nacimiento', '$sexo', '$domicilio', '$localidad', '$departamento', 
...
$sql_den="select id_bru from epi.brucel_can where id_denuncia=$id_denuncia";
$res_den =sql($sql_den, "Error consulta t5") or fin_pagina();
if ($res_den->recordcount()>0) $id_bru=$res_den->fields['id_bru'];

if ($id_bru) {
		
			$q_lep="select * from epi.brucel_can where id_denuncia=$id_denuncia";
			$res_lep=sql($q_lep, "Error consulta t1") or fin_pagina();
			if($res_lep->RecordCount()!=0){
					$id_bru=$res_lep->fields['id_bru'];
...
<input type="hidden" value="<?=$id_bru?>" name="id_bru">
