...
///////////////////////////////////////// agregado por Gabriel //////////////////////////////////////////////
	...
	if ($_POST["sexo"]) $lext["ext"]["sexo"]=$_POST["sexo"];
	if ($_POST["id_asignado"]) $lext["ext"]["id_asignado"]=$_POST["id_asignado"];
	if ($_POST["id_efectivo"]) $lext["ext"]["id_efectivo"]=$_POST["id_efectivo"];
	...
	if ($h_id_legajo){
		...
		if (!$_POST["leg_guardar"]){
			...
			$consulta="select id_legajo, hr_entra, hr_sale, ubicacion, id_afjp, id_usuario, login,
				...
				otros_conocimientos, exhibe_titulos, sexo, id_asignado, id_efectivo, codigo_postal, in_categoria, in_sueldo_inicial, in_ocupacion ";
		...
		$lext["ext"]=array("id_legajo"=>"", "estado_civil"=>"0", "fecha_estado_civil"=>date("Y-m-d"), "nacionalidad"=>"argentino",
			...
			"profesion"=>"", "estudios"=>"", "otros_conocimientos"=>"", "exhibe_titulos"=>"n", "sexo"=>"", "id_asignado"=>"", "id_efectivo"=>"", "codigo_postal"=>"",
	...
	if (!$error) {
		if ($_POST["leg_nuevo"] == "1") {
			...
			$consulta="insert into legajos_ext (id_legajo, estado_civil, fecha_estado_civil, cedula_identidad, nacionalidad,
			  ...
			  in_art, in_beneficiario, in_categoria, profesion, estudios, codigo_postal, otros_conocimientos, exhibe_titulos, sexo,id_asignado, id_efectivo) values (";
			$consulta.=$id_legajo.", "
				...
				.(($lext["ext"]["sexo"])?"'".$lext["ext"]["sexo"]."'":"null").", "
				.(($lext["ext"]["id_asignado"])?"'".$lext["ext"]["id_asignado"]."'":"null").", "
				.(($lext["ext"]["id_efectivo"])?"'".$lext["ext"]["id_efectivo"]."'":"null").")";
		...
		elseif ($_POST["leg_nuevo"] == "0") {
			...
			$sql .= "caja_ahorro_pesos_nro='$caja_ahorro',tipo_liq=$tipo_liq,tipo_jub=1, ";
			...
			if ($rta_consulta->recordCount()==1){
				$consulta="update legajos_ext set "
					...
					.", sexo=".(($lext["ext"]["sexo"])?"'".$lext["ext"]["sexo"]."'":"null")
					.", id_asignado=".(($lext["ext"]["id_asignado"])?"'".$lext["ext"]["id_asignado"]."'":"null")
					.", id_efectivo=".(($lext["ext"]["id_efectivo"])?"'".$lext["ext"]["id_efectivo"]."'":"null")
...
function form_modificar($nuevo = false,$id = "") {
  ...
  $id_afjp=$_POST['leg_afjp'];
	...
	echo "<tr><td style=\"border:$bgcolor3;cursor:hand;\" colspan=2 align=center id=mo onclick=\"muestra_tabla(document.all.tabla_datos_personales,0);\">
		...
		<font size=+1>Legajo de personal (Nro: $dni)</font></td></tr>";
	...
	echo "</tr><tr>";
	echo "<td align=right><font color='red'>*</font><b>DNI:</b></td>";
	...
	<tr>
		...
		<td align=right><font color='red'>*</font><b>Sexo:</b></td>
		<td align=left>
			<select name=sexo Style="width=195px">
			  <option value=-1>Seleccione</option>
			  <option value=Masculino <?if ($lext["ext"]["sexo"]=='Masculino') echo "selected"?>>Masculino</option>			  
			  <option value=Femenino <?if ($lext["ext"]["sexo"]=='Femenino') echo "selected"?>>Femenino</option>
			  <?echo $lext["ext"]["sexo"];			?>  
			</select>
		</td>
	</tr>
	<?echo "<tr><td align=right><font color='red'>*</font><b>Fecha de nacimiento:</b></td>";
	echo "<td align=left><input type=text name=leg_fecha_nacimiento value='$fecha_nacimiento' size=10 maxlength=10>".link_calendario("leg_fecha_nacimiento")?>
	<?function CalculaEdad( $fecha ) {
		list($Y,$m,$d) = explode("-",$fecha);
		return( date("md") < $m.$d ? date("Y")-$Y-1 : date("Y")-$Y );
		}
	echo "<b>Edad: ".CalculaEdad (fecha_db($fecha_nacimiento))." Años.</b>"?>
	</td>
	<?echo "</tr><tr>";
	...
	echo "</tr><tr>";
	echo "<td align=right><font color='red'>*</font><b>Teléfono particular:</b></td>";
	...
	?>
	<tr>
		<td style=\"border:$bgcolor3;cursor:hand;\" colspan=4 align=center id=mo onclick="muestra_tabla(document.all.tabla_datos_laborales,1);">
	...
	<tr><td colspan="4" align="center">
	  ...
	  <table>
		  ...
		  <td align=right><font color='red'>*</font><b>Fecha de ingreso:</b></td>
	      <td align=left><input type=text name=leg_fecha_ingreso value='<?=fecha($fecha_ingreso)?>' size=10 maxlength=10><?=link_calendario("leg_fecha_ingreso")?>
	      <?echo "<b>Antiguedad: ".CalculaEdad ($fecha_ingreso)." Años.</b>"?></td>
	  	</tr>
	    ...
	    <tr>
	    	...
	    	<td align="left" colspan="3"><input type=text size=90 name=t_observaciones_egreso value='<?=$lext["ext"]["baja_observaciones"]?>'></td>
	    </tr>
	    	    
	    <tr><td colspan="5" align="left"><hr><b>Tipo de Liquidación:</b></td></tr>
	    <tr><td align="left" colspan="2">
            <INPUT TYPE='RADIO' name='leg_tipo' value='1' <? if ($tipo_liq==1) echo 'checked'?>>&nbsp;<b>Sueldos y jornales</b>
        </td></tr>
        <tr><td align="left" colspan="2">
             <INPUT TYPE='RADIO' name='leg_tipo' value='2' <?if ($tipo_liq==2) echo 'checked'?>>&nbsp;<b>Honorarios por Producto</b>
        </td></tr>
        
        <tr><td colspan="5" align="left"><hr><b>Datos de Contratacion:</b></td></tr>
        <tr>
			<td colspan="2"><font color='red'>*</font><b>Regimen:</b>			
            ...
            <select name="leg_tarea">
             <? for ($i=0;$i<$cant_tareas;$i++) {
              ...
              <option value='<?=$id_tarea_res?>'
               ...
               <?//=$res_q_tarea->fields['codigo']."-".$res_q_tarea->fields['nombre_tarea'];?>
               <?=$res_q_tarea->fields['nombre_tarea'];?>
              </option>
             <? $res_q_tarea->MoveNext(); }?>
             </select>
			</td>
			<td align="left"><font color='red'>*</font><b>Categor&iacute;a:</b>
			<?$rta_consulta=sql("select * from categorias order by descripcion, nombre", "No se pudieron obtener las categorías") or fin_pagina();
       		$categorias_nombre=array();
       		$categorias_id=array();
       		
       		while (!$rta_consulta->EOF){
       			$categorias_nombre[]=$rta_consulta->fields["nombre"]. " - " .$rta_consulta->fields["descripcion"];
        ...
        <tr>
			<td colspan="2">
				<font color='red'>*</font><b>Funcion:</b>
        <? $q_calif="select * from personal.calificacion order by nombre_calificacion";
        ...
        ?>
          <select name="leg_calif">
        	...
        	<b>Establecimiento:</b>
        	<?
        		$rta_consulta=sql("select * from personal.distrito order by nombre", "No se pudo conseguir la lista de distritos") or fin_pagina();
        ...
        <tr>
			<td colspan="2"><font color='red'>*</font><b>Programa Asignado:</b>			
			   <?echo $id_asignado;
			   $q_tarea="select * from personal.programas";
			   $res_q_tarea=sql($q_tarea, "Error al traer los datos de las tareas") or fin_pagina();
			   $cant_tareas=$res_q_tarea->RecordCount();?>
            <select name="id_asignado" style ="width:300px;">
             <? for ($i=0;$i<$cant_tareas;$i++) {
                $id_tarea_res=$res_q_tarea->fields['id_programas'];?>
              <option value='<?=$id_tarea_res?>'
               <?if ($lext["ext"]["id_asignado"]==$id_tarea_res) echo "selected"?>>
               <?=$res_q_tarea->fields['descripcion'];?>
              </option>
             <? $res_q_tarea->MoveNext(); }?>
             </select>
			</td>
			<td colspan="2"><font color='red'>*</font><b>Programa Efectivo:</b>			
			   <?$q_tarea="select * from personal.programas order by descripcion";
			   $res_q_tarea=sql($q_tarea, "Error al traer los datos de las tareas") or fin_pagina();
			   $cant_tareas=$res_q_tarea->RecordCount();?>
            <select name="id_efectivo" style ="width:300px;">
             <? for ($i=0;$i<$cant_tareas;$i++) {
                $id_tarea_res=$res_q_tarea->fields['id_programas'];?>
              <option value='<?=$id_tarea_res?>'
               <?if ($lext["ext"]["id_efectivo"]==$id_tarea_res) echo "selected"?>>
               <?=$res_q_tarea->fields['descripcion'];?>
              </option>
             <? $res_q_tarea->MoveNext(); }?>
             </select>
      ...
      <tr>
       	<td align="left" colspan="2" nowrap ><b>Servicio:</b><input size="60" type="text" name="t_sector" value="<?=$lext["ext"]["in_sector"]?>"></td>
       	<td align="left" colspan="2" nowrap ><b>Ocupaci&oacute;n:</b><input size="60" type="text" name="t_ocupacion" value="<?=$lext["ext"]["in_ocupacion"]?>"></td>
      </tr>
      
      <tr><td colspan="5" align="left"><hr><b></b></td></tr>
           
      <tr>
       	<td align="left" colspan="4"><b>Observaciones del ingreso:</b>
       	<textarea name="ta_observaciones_ingreso" cols="90" rows="4"><?=$lext["ext"]["in_observaciones"]?></textarea></td>
      </tr>
      
      <tr>
		 <td align="left" colspan="2"><b>Sueldo inicial: $</b><input type="text" name="t_sueldo_inicial" value="<?=number_format($lext["ext"]["in_sueldo_inicial"], 2, ",", "")?>" onchange="this.value=this.value.replace(',','.'); return control_numero(this, 'Sueldo inicial');"></td>
     	<td colspan="2" nowrap>
        	<?
        		$horarios=array();
		...
		<td align="center" colspan="2">
			<table width="100%"  id="tabla_disciplina" style="display:none;"><tr><td align="center">
				...
				for(; $i<count($lext["ausentismo"]); $i++){
					?>
					<tr align="center">
						<td width="25%"><?=g_draw_range_select("sel_agno_".$i, (($lext["ausentismo"][$i]["agno"])?$lext["ausentismo"][$i]["agno"]:date("Y")), 2010, date("Y"), 1, "><option value='-borrar-'>-borrar-<b></b")?></td>
...
<tr>
	...
	?>
	   ...
	   <? $i=1;
        ...
        ?>
	    <tr>
	      <td align="center" colspan="2"><input type="button" name=nuevo value='Nuevo' onclick="<?=$link1?>"></td>
	    </tr>
